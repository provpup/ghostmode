...
class TimeJudge
  ...
  def self.run_checkpoint_times(run)
    ...
    run_candidate_milestones.each_cons(2) do |race_segment|
      ...
      relative_times << run_relative_time.abs
    end
  ...
  def self.split_times(run1, run2)
    ...
    run1_candidate_milestones = []
    previous_candidate_milestone = nil
    checkpoints.each do |checkpoint|
      milestone = GpsPoint.closest_point(checkpoint, previous_candidate_milestone, run1.gps_points)
      run1_candidate_milestones << milestone
      previous_candidate_milestone = milestone
    end


    run2_candidate_milestones = []
    previous_candidate_milestone = nil
    checkpoints.each do |checkpoint|
      milestone = GpsPoint.closest_point(checkpoint, previous_candidate_milestone, run2.gps_points)
      run2_candidate_milestones << milestone
      previous_candidate_milestone = milestone
    end

run1_relative_times = run_checkpoint_times(run1).values
run2_relative_times = run_checkpoint_times(run2).values
...
milestones = run1_relative_times.zip(run2_relative_times)
    ...
    milestones.each do |rt_pair|
      checkpoint_split_times << rt_pair.last - rt_pair.first
    end
